...
色彩風格新增三款，並將粉紅色／若竹色更名為櫻花粉／若竹綠。包含預設共六種風格，每種都有淺色與暗色版本；舊設定及備份仍可載入，跟隨系統保留所選風格。這批色彩調整已納入本機 v1.2.0 安裝包，尚未上傳 Release。
...
首頁釘選區內的卡片現在平均分配可用高度，兩張卡片等高並填滿外框；標題置頂、數值置中，日期及進度位於底部。這次內部卡片高度調整已納入本機 v1.2.0 安裝包。

倒數／正數的目標時間與提醒時間支援直接輸入 `HH:mm`，或開啟 24 小時鐘面點選／拖曳時、分；按確定套用，取消保留原值。五種分類、滑動選取與鐘面選時已納入本機 v1.2.0 安裝包，尚未上傳 Release。

最新編輯器的計數方向／顯示方式採滑動式選取，首頁釘選與假日略過採開關；年度國曆月日提供日期選擇彈窗，預覽共用子功能頁倒數卡片版面。這批介面調整已納入本機 v1.2.0 安裝包，尚未上傳 Release。

新增／編輯事件現在先以膠囊式滑動選項選擇倒數或正數，再填寫名稱、日期與提醒；切換時同步顯示模式說明與預覽。計數方向移至最前面的調整已納入本機 v1.2.0 安裝包，尚未上傳 Release。
...
依 Windows 顯示縮放換算介面尺寸；其他解析度依螢幕高度套用對應級距，工作區不足時等比例縮小。自動預設尺寸最高採用 2K 級距的 1792×1280，4K／5K／8K 不再繼續放大；仍可手動拉大或最大化，重新從系統匣開啟也會保留調整後的大小。7:5 比例、預設尺寸上限及首頁外框對齊均已納入本機 v1.2.0 安裝包，尚未上傳 Release。

首頁釘選卡依狀態變色：一般倒數為藍色、7 天內為橙色、當天／到期為玫紅色、正數累計為綠色，已完成或日期待確認為灰色；底色、邊框、數字、圖示與進度條同步配色，並保留狀態文字。支援六種主題，這次配色調整已納入本機 v1.2.0 安裝包，尚未上傳 Release。
