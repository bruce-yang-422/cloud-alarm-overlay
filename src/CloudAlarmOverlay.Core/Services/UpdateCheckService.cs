...
public sealed class UpdateCheckService(ISettingsRepository settings, HttpClient client) : IUpdateCheckService
{
    ...
    {
        var info=await GetManifestAsync(cancellationToken);
        var latest=info.LatestVersion;
        return new Version(latest.Major,latest.Minor,Math.Max(0,latest.Build),Math.Max(0,latest.Revision))>CurrentVersion?info:null;
    }
    public async Task<UpdateInfo> GetManifestAsync(CancellationToken cancellationToken=default)
    {
