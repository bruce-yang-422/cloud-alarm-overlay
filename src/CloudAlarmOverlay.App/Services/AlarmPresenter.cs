...
public sealed class AlarmPresenter(IRuntimeStore runtime,IDeviceIdentityService identity,IAckCodeGenerator codes,ChangeSignal changes,
    IHostApplicationLifetime lifetime,NotificationPreferences preferences,ISoundService sound,CloudAlarmOverlay.Core.Repositories.ISystemEventStore events):IAlarmPresenter,IDisposable
{
    ...
    public async Task ShowAsync(string occurrenceId,AlarmTask task,DateTime scheduledAt,bool preview,CancellationToken ct=default)
    {
        ...
        try
        {
            ...
            await dispatcher.InvokeAsync(()=>{
                ...
                window=new AlarmWindow(new AlarmViewModel(task,codes.Generate(),preview),task.Level,large?0:smallCount++,flashMs,colorMode,colorScheme);
                window.Show();
