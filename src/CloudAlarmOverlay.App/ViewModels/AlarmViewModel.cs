...
public partial class AlarmViewModel(AlarmTask task,string code,bool preview,string? caption=null):ObservableObject
{
    ...
    public string Instruction=>caption is not null?"閱讀後按下確認，繼續番茄鐘流程。":RequiresCode?"請閱讀以上內容，輸入 9 位數字確認碼；可輸入 xxx-xxx-xxx 或連續 9 位數字。":task.Level==AlarmLevels.Low?"10 秒後自動關閉；滑鼠移入可暫停。":"閱讀後，按下確認完成簽收。";
